...
class id group ;
model frequency = group time group*time chronicity age time*chronicity age*time / noint dist=poisson ;
...
proc glimmix data=lda2.aculda2;
class id;
model frequency = group time group*time age  age*time /noint dist=poisson;
random intercept  /type=un subject=ID g gcorr v vcorr;
run;
...
class id;
model frequency = group time group*time age  age*time/noint dist=poisson solution;
