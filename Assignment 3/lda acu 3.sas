...
/* Long to wide format*/
proc transpose data=lda3.acu2 out=lda3.acu2b prefix=severity;
    by id age chronicity group;
    id time;
    var severity ;
run;

proc sort data=lda3.acu2b;
by group;
run;

/*Overview of missigness table*/
proc mi data=lda3.acu2b seed=675938 simple nimpute=0;
title ’standard EM’;
...
/*LMM*/
/*method=ml*/
proc mixed data=lda3.acu2 method=ml nobound;
...
run;
/*method=reml*/
proc mixed data=lda3.acu2 method=reml nobound;
class logtimeclass group id;
model severity= age chronicity group*logtime age*logtime chronicity*logtime/ solution;
random intercept logtime  /type=un subject=ID g gcorr v vcorr;
repeated logtimeclass / type=un subject=ID r rcorr;
...
proc mi data=lda3.acu2b seed=486048 simple out=lda3.acu3 nimpute=20 round=0.1;
...
proc mixed data=lda3.acu3b method=ml asycov covtest nobound;
class logtimeclass group id;
by _imputation_;
...
proc mi data=lda3.acu2b seed=486048 simple out=lda3.acu4 nimpute=20 round=0.1;
...
%dropout(data=lda3.acu4b,id=id,time=logtime,response=severity,out=lda3.acu4c);

proc gee data=lda3.acu4c;
by _imputation_;
